            ...
            mainGroup.forEach { (type, groupedData) ->
                ...
                subGroup.forEach { (subType, subGroupedData)->
                    ...
                    items(items = subGroupedData) {
                         ...
                         */
                        if (remember { derivedStateOf { listState.firstVisibleItemIndex } }.value < firstItemIndex.value) { // move to start
                            if (remember { derivedStateOf { listState.firstVisibleItemIndex } }.value > 0) {
                                header.value = dataList[remember { derivedStateOf { listState.firstVisibleItemIndex }.value } - 1].type
                            }
                        }
                        else if (remember { derivedStateOf { listState.firstVisibleItemIndex } }.value > firstItemIndex.value) { // move to the end
                            /*
                             ...
                             */
                            if (remember { derivedStateOf { listState.firstVisibleItemIndex } }.value - firstItemIndex.value != 2){
                                header.value = dataList[remember { derivedStateOf { listState.firstVisibleItemIndex } }.value].type
                            }
                        }
                        firstItemIndex.value = remember { derivedStateOf { listState.firstVisibleItemIndex } }.value
                    }
                }
            }
        }
